...
@Describe: CRMICM   特征选择核心算法 基于RMIC的 CMIM改进特征选择算法
"""
...
from ...association.mic import PairwiseRMIC as PairwiseAssoc
# from ...association.mic import PairwiseMIC as PairwiseAssoc
...
def cal_assoc(x, y):
    ...
    return PairwiseAssoc(x, y).cal_assoc()
...
class CondAssoc(object):
    ...
    def cal_cond_assoc(self) -> float:
        ...
        for label in xs_labels:
            ...
            # assoc_sub = 0.0 if assoc_sub < 0.1 else assoc_sub  # TODO
            gain += prob * assoc_sub
        return gain
...
class CondRMICMaximization(object):
    ...
    def _recog_redundant_markov_features(self, F: set, S: list, R: set, U: set, f: int, thres: float = 0.0) -> set:
