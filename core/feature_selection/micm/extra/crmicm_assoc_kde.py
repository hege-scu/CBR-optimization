...
from ...association.mic_kde import PairwiseRMIC
...
class CondAssoc(object):
    ...
    def _binning_xs(xs):
        ...
        # ---
        q = int(len(xs) // 70)  # FIXME: 其他为80, dry_gas为70
    ...
    def cal_cond_assoc(self) -> float:
        ...
        for label in xs_labels:
            ...
            # assoc_sub = 0.0 if assoc_sub < 0.05 else assoc_sub  # TODO
            gain += prob * assoc_sub
        return gain
...
class CondRMICMaximization(object):
    ...
    def _recog_redundant_markov_features(self, F: set, S: list, R: set, U: set, f: int, thres: float = 0.05) -> set:
