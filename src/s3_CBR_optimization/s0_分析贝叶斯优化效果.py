...
if __name__ == "__main__":
    ...
    # note: 所有theta样本需要进行归一化
    # scaler_theta = MinMaxScaler()
    # Theta_lib_samples = scaler_theta.fit_transform(Theta_lib_df.values)
    ...
    # todo: 改一下路径哈
    df_gas = pd.read_csv(f"{BASE_DIR}/case_fcc_knn_gas/results.csv")
    df_liq = pd.read_csv(f"{BASE_DIR}/case_fcc_knn_liq/results.csv")
    df_coke = pd.read_csv(f"{BASE_DIR}/case_fcc_knn_coke/results.csv")
    ...
    for i in range(3):
        ...
        plt.xlabel("Euclidean norm of $(\\theta_{\\rm obs} - \\theta^*_{\\rm MAP})$")
        ...
        if i in [0, 1]:
            ...
            plt.ylim([-0.8, 0.2])
        plt.grid(True, linewidth=0.5, zorder=-10)
        plt.tight_layout()
    plt.savefig("优化前后收率变化.png", dpi=450)
    ...
    #     plt.xlabel(r"Euclidean norm of $\theta$")
    #     plt.yticks([0, 1], [r"$\theta_{\rm obs}$", r"$\theta_{\rm MAP}^*$"])
    #     plt.ylim([-0.5, 1.5])
    # plt.tight_layout()
    # plt.savefig("优化前后Theta范数变化.png", dpi=450)